...
import {removePermissions} from './common-renderer'
...
function Options() {
	...
	useEffect(() => {
		...
		ipcRenderer
			.invoke('storage-get', ['private_key', 'protocol_handler', 'notifications'])
			.then(results => {
	...
	async function loadPermissions() {
		// Загрузка разрешений
		let {policies = {}} = await ipcRenderer.invoke('storage-get', ['policies'])
		let list = []
						...
						>
							<form style={{display: 'flex', flexDirection: 'row', gap: '10px'}}>
								...
								{askPassword === 'decrypt/save' ? (
									<button onClick={decryptPrivateKeyAndSave} disabled={!password}>
										decrypt key
									</button>
								) : askPassword === 'encrypt/display' ? (
									<button
    ...
    examples:
      ...
      - https://nostr.band/{raw}
			...
			<div>
				...
				{!!policies.length && (
					<div style={{display: 'flex'}}>
							...
							<tbody>
								{policies.map(({host, type, accept, conditions, created_at}, index) => (
									<tr key={host + type + accept + JSON.stringify(conditions)}>
										...
										<td>
											{new Date(created_at * 1000).toISOString().split('.')[0].split('T').join(' ')}
										</td>
										<td>
											<input
												type="checkbox"
						...
						{selectedItems.length > 0 ? (
							<button style={{marginLeft: '0.5rem'}} onClick={handleMultiRevoke}>
								revoke
							</button>
						) : null}
					</div>
				...
				{!policies.length && (
					<div style={{marginTop: '5px'}}>no permissions have been granted yet</div>
				)}
			</div>
		</>
	)
	...
	function decryptPrivateKeyAndSave(ev) {
		...
		try {
			...
			// Заменяем browser.storage.local.set на прямой вызов IPC
	...
	async function saveKey() {
		...
		if (privKeyInput === '') {
			await ipcRenderer.invoke('storage-set', {
				private_key: null
			})
			showMessage('Private key deleted!')
			return
		}
	...
	function isKeyValid() {
		// Пустая строка разрешена для удаления ключа
	...
	async function requestBrowserNotificationPermissions() {
		// В Electron мы просто возвращаем true
		return true
	}

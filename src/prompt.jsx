...
import {PERMISSION_NAMES} from './common-renderer'
...
function Prompt() {
	...
	return (
		<>
			<b style={{display: 'block', textAlign: 'center', fontSize: '200%'}}>{host}</b>{' '}
			<p style={{margin: 0}}>
	...
	function authorizeHandler(accept, conditions) {
		return function (ev) {
			...
			// Отправка ответа на запрос через IPC
			ipcRenderer.send('prompt-response', {
				accept,
				id,
				host,
				type,
